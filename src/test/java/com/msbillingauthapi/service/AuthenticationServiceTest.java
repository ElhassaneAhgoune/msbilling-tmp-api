...
class AuthenticationServiceTest {
    ...
    void authenticate_validRequest_returnsJwtToken() {
        ...
        when(jwtService.generateToken(username,role)).thenReturn("mocked-jwt-token");
        when(userRepository.findByUsername(username)).thenReturn(Optional.of(TestUserBuilder.userBuilder().build()));
        ...
        verify(jwtService).generateToken(username,role);
    }

    @Test
    void refreshToken_validToken_returnsNewAccessToken() {
        final var refreshToken = TestRefreshTokenBuilder.refreshTokenBuilder().withRandomId().withTestUser().build();

        when(refreshTokenRepository.findByIdAndExpiresAtAfter(eq(refreshToken.getId()), any(Instant.class)))
                .thenReturn(Optional.of(refreshToken));

        when(jwtService.generateToken(refreshToken.getUser().getUsername(),refreshToken.getUser().getRole().getRoleName())).thenReturn("new-jwt-token");
        ...
        verify(jwtService).generateToken(refreshToken.getUser().getUsername(),refreshToken.getUser().getRole().getRoleName());
    }
