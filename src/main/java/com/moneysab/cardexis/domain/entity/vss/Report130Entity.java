...
/**
 * Entity for VSS-130 Reimbursement Fees Report data.
 * 
 * @author EL.AHGOUNE
 * @version 1.0.0
 * @since 2024
 */
@Entity
@Table(name = "vss_report_130", 
       indexes = {
           @Index(name = "idx_vss_report_130_currency", columnList = "settlement_currency"),
           @Index(name = "idx_vss_report_130_proc_date", columnList = "processing_date"),
           @Index(name = "idx_vss_report_130_file", columnList = "source_file_name")
       })
...
public class Report130Entity {
    ...
    @Column(name = "reimbursement_fee_credits", precision = 15, scale = 2)
    private BigDecimal reimbursementFeeCredits;

    @Column(name = "reimbursement_fee_debits", precision = 15, scale = 2)
    ...
    @Size(max = 100)
    @Column(name = "fee_category", length = 100)
